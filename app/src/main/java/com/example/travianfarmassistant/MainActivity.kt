...
class MainActivity : Activity() {
    ...
    private fun visitNextVillageForScan() {
        ...
        val server = normalizeServer(serverInput.text.toString())
        val targetUrl = "$server/dorf1.php?newdid=$id"
        logEvent("UI: [$progress] URL TARGET → $targetUrl")
    ...
    private fun collectCurrentVillageData() {
        ...
        val js = """
            (() => {
                ...
                const activeId = activeEntry?.getAttribute('data-did') || '';
                const activeName = clean(activeEntry?.querySelector('.name')?.textContent || '');
