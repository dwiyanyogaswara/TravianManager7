...
class MainActivity : Activity() {
    ...
    private fun loadVillagesForUi() {
        ...
        villageScanIndex = 0
        villageScanExpected = 0
        farmStatus.text = "Mencari daftar village..."
        logEvent("UI: mulai scan village satu per satu")
        ...
        val js = """
            (() => {
                ...
                const add = (href, nameHint='') => {
                    if (!href) return;
                    const abs = new URL(href, location.href).href;
                    const m = abs.match(/[?&]newdid=(\d+)/i);
                    if (!m) return;
                    const id = m[1];
                    if (seen.has(id)) return;
                    seen.add(id);
                    let name = clean(nameHint);
                    if (!name) name = 'Village ' + id;
                    villages.push({id:id, name:name, url:abs});
                };
                const scan = root => {
                    if (!root) return;
                    // Prioritas: link village yang benar-benar dapat diklik.
                    for (const a of root.querySelectorAll('a[href]')) {
                        const href = a.getAttribute('href') || '';
                        if (!/[?&]newdid=\d+/i.test(href)) continue;
                        const entry = a.closest('.listEntry, li, .village, .listEntryVillage');
                        const nameEl = entry?.querySelector('.name, .villageName, .name');
                        const name = clean(nameEl?.textContent || a.getAttribute('title') || a.textContent || '');
                        add(href, name);
                    }
                    // Beberapa layout menyimpan did di data attribute entry.
                    for (const entry of root.querySelectorAll('.listEntry,[data-did],[data-village-id],[data-villageid]')) {
                        const id = entry.getAttribute('data-did') || entry.getAttribute('data-village-id') || entry.getAttribute('data-villageid');
                        if (!id) continue;
                        const link = entry.querySelector('a[href]');
                        if (!link) continue;
                        const name = clean(entry.querySelector('.name,.villageName')?.textContent || entry.getAttribute('data-name') || link.getAttribute('title') || link.textContent || '');
                ...
                const mCount = bodyText.match(/VILLAGES\s+(\d+)\s*\/\s*\d+/i) || bodyText.match(/VILLAGES\s*\(?\s*(\d+)\s*\/\s*\d+\)?/i);
                const expected = mCount ? parseInt(mCount[1],10) || 0 : 0;
                const current = location.href.match(/[?&]newdid=(\d+)/i);
                if (current && !seen.has(current[1])) {
                    // Jika halaman sekarang adalah village yang valid, masukkan juga.
                    const titleName = clean(document.querySelector('#villageName, .villageName, h1')?.textContent || document.title || '');
                    add(location.href, titleName);
                }

                // Fallback profile tetap dipakai hanya untuk mendapatkan LINK TARGET.
                // Setelah itu setiap target tetap dibuka satu per satu di WebView.
                const finish = result => AndroidFarm.onVillageTargetsResult(JSON.stringify(result));
                if (expected > 0 && villages.length < expected) {
                    ...
                    if (profileLink) {
                        fetch(profileLink, {credentials:'include', cache:'no-store'})
                            .then(r => r.text())
                            .then(html => {
                                const doc = new DOMParser().parseFromString(html, 'text/html');
                                [doc.querySelector('#villageList'), doc.querySelector('#sidebarBoxVillagelist'), doc.body].filter(Boolean).forEach(scan);
                                finish({villages, expected});
                            })
                            .catch(() => finish({villages, expected}));
                    } else {
                        finish({villages, expected});
                    }
                } else {
    ...
    private fun visitNextVillageForScan() {
        ...
        // Klik link village yang benar-benar ada di sidebar. Jika layout Travian
        // tidak menyediakan link di DOM saat itu, fallback ke URL newdid.
        val js = """
            (() => {
                const id = '$id';
                ...
                location.href = '${normalizeServer(serverInput.text.toString())}/dorf1.php?newdid=' + id;
                return 'fallback';
            })();
        """.trimIndent()
        webView.evaluateJavascript(js, null)
    ...
    private fun collectCurrentVillageData() {
        ...
        val js = """
            (() => {
                ...
                let name = clean(
                    document.querySelector('#villageName')?.textContent ||
                    document.querySelector('.villageName')?.textContent ||
                    document.querySelector('#sidebarBoxVillagelist .selected .name')?.textContent ||
