    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        webView.webViewClient = object : WebViewClient() {
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                // Saat LOAD VILLAGE aktif, setiap navigasi adalah bagian dari
                // scanner. Jangan jalankan alur Farm List/login biasa karena
                // scanner harus menunggu halaman village selesai dimuat.
                if (villageScanActive) {
                    // Jangan membaca data sebelum URL benar-benar pindah ke village target.
                    // WebView kadang masih menampilkan halaman lama beberapa ratus ms setelah
                    // klik anchor Travian dipicu.
    ...
    private fun collectCurrentVillageData() {
        ...
        val js = """
            (() => {
                ...
                const fields = [];
                for (const el of document.querySelectorAll('#resourceFieldContainer .level, #resourceFieldContainer [class*="level"], #resourceFieldContainer a')) {
                    const t = clean(el.textContent || '');
                    const m = t.match(/(?:level|lvl)\\s*(\\d+)/i) || (el.className || '').toString().match(/(?:^|\\s)level(\\d+)(?:\\s|$)/i);
                    if (m) fields.push(parseInt(m[1],10));
                }
