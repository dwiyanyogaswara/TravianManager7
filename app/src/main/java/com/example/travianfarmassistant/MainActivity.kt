...
class MainActivity : Activity() {
    ...
    private fun loadVillagesForUi() {
        ...
        logEvent("UI: mulai memuat daftar village")
            ...
            (async () => {
                ...
                if (expected > 0 && villages.length < expected) {
                    try {
                        ...
                        // UI akan menerima daftar yang berhasil dibaca dari halaman utama.
                    }
                }

                // evaluateJavascript() tidak menunggu Promise.
                // Kirim hasil async kembali ke Android melalui JavascriptInterface.
                try {
                    AndroidFarm.onVillageListResult(JSON.stringify({villages:villages,expected:expected}));
                } catch (e) {
                    AndroidFarm.onVillageListResult(JSON.stringify({villages:[],expected:expected,error:String(e)}));
                }
            })().catch(e => {
                AndroidFarm.onVillageListResult(JSON.stringify({villages:[],expected:0,error:String(e)}));
            });
        """.trimIndent()

        // Jangan membaca hasil evaluateJavascript di sini karena JavaScript di atas async.
        webView.evaluateJavascript(js, null)
    }
    ...
    inner class FarmBridge {
        ...
        fun onVillageListResult(result: String) {
            runOnUiThread {
                ...
                if (unique.isEmpty()) {
                    farmStatus.text = if (error.isNotBlank()) {
                        "Gagal memuat daftar village: $error"
                    } else {
                        "Village belum terlihat. Buka Travian/login terlebih dahulu."
                    }
                    logEvent("UI: daftar village tidak ditemukan${if (error.isNotBlank()) ": $error" else ""}")
