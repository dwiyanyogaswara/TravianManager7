...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        webView.webViewClient = object : WebViewClient() {
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                // LOAD VILLAGE harus punya prioritas atas scheduler/service.
                // Kalau service diproses lebih dulu, forwardPageFinished() dapat
                // langsung mengarahkan WebView kembali ke alur automation sehingga
                // scanner tidak pernah sempat membaca village satu per satu.
                if (villageScanActive) {
                    handler.postDelayed({ collectCurrentVillageData() }, 900)
                    return
                }
    ...
    private fun loadVillagesForUi() {
        ...
        logEvent("UI: mulai scan village satu per satu dari sidebar Travian")

        // Travian Legends menyimpan setiap village di:
        // #sidebarBoxVillagelist .villageList .dropContainer .listEntry
        // dan ID village berada di data-did. Jangan bergantung hanya pada
        // href=newdid karena beberapa layout/group tidak mengekspos href itu
        ...
        val js = """
            (() => {
                ...
                const villages = [];
                const seen = new Set();
                const add = (id, name, href) => {
                    id = String(id || '').trim();
                    if (!/^\\d+$/.test(id) || seen.has(id)) return;
                    seen.add(id);
                    villages.push({
                        id,
                        name: clean(name) || ('Village ' + id),
                        url: href || ('dorf1.php?newdid=' + id)
                    });
                };

                const root = document.querySelector('#sidebarBoxVillagelist');
                if (root) {
                    // Canonical current Travian structure.
                    for (const entry of root.querySelectorAll('.villageList .dropContainer .listEntry, .villageList .listEntry, .listEntry[data-did]')) {
                        const id = entry.getAttribute('data-did');
                        if (!id) continue;
                        const nameEl = entry.querySelector('.name');
                        const a = entry.querySelector('a[href]');
                        add(id, nameEl?.textContent || '', a?.getAttribute('href') || '');
                    }

                    // Older/layout fallback: li/a and newdid.
                    for (const li of root.querySelectorAll('li')) {
                        const a = li.querySelector('a[href]');
                        if (!a) continue;
                        const href = a.getAttribute('href') || '';
                        const m = href.match(/[?&]newdid=(\\d+)/i);
                        if (!m) continue;
                        add(m[1], li.querySelector('.name')?.textContent || a.textContent || '', href);
                    }
                }

                // Fallbacks outside the sidebar.
                for (const root2 of [document.querySelector('#villageList'), document.querySelector('#side_info')].filter(Boolean)) {
                    for (const el of root2.querySelectorAll('[data-did], a[href*="newdid="]')) {
                        const id = el.getAttribute('data-did') || (el.getAttribute('href') || '').match(/[?&]newdid=(\\d+)/i)?.[1];
                        if (!id) continue;
                        const a = el.matches('a') ? el : el.querySelector('a[href]');
                        add(id, el.querySelector?.('.name')?.textContent || el.getAttribute('title') || a?.textContent || '', a?.getAttribute('href') || '');
                    }
                }
                ...
                const expected = countMatch ? parseInt(countMatch[1], 10) || 0 : 0;

                AndroidFarm.onVillageTargetsResult(JSON.stringify({villages, expected}));
            })();
        """.trimIndent()
    ...
    private fun handleVillageTargetsResult(rawJson: String) {
        ...
        villageScanExpected = json?.optInt("expected", 0) ?: 0

        if (unique.isEmpty()) {
            villageScanActive = false
            farmStatus.text = "Daftar village tidak ditemukan."
            logEvent("UI: target village tidak ditemukan")
            return
        }

        if (villageScanExpected > 0 && unique.size < villageScanExpected && villageScanRetry < 5) {
            villageScanRetry++
            farmStatus.text = "Daftar village baru ${unique.size}/${villageScanExpected}; mencari lagi (${villageScanRetry}/5)..."
            logEvent("UI: daftar village belum lengkap ${unique.size}/${villageScanExpected}; retry ${villageScanRetry}/5")
            handler.postDelayed({ loadVillagesForUi() }, 1200)
            return
        }
        ...
        logEvent("UI: ${villageScanTargets.size}${if (villageScanExpected > 0) "/$villageScanExpected" else ""} target village ditemukan; mulai klik satu per satu")
    ...
    private fun visitNextVillageForScan() {
        ...
        val (id, hintName) = villageScanTargets[villageScanIndex]
        val progress = "${villageScanIndex + 1}/${villageScanTargets.size}"
        farmStatus.text = "Klik village $progress — $hintName"
        logEvent("UI: klik village $progress — $hintName (ID $id)")
        villageScanPageRetry = 0

        // Jangan hanya memanggil a.click(). Pada beberapa layout Travian, anchor
        // dipasang sebagai handler JS sehingga click() tidak selalu memicu navigasi.
        // Ambil href asli dari baris village lalu paksa WebView menavigasi ke URL itu.
        val idJson = JSONObject.quote(id)
        val js = """
            (() => {
                const id = $idJson;
                const root = document.querySelector('#sidebarBoxVillagelist');
                const entries = root ? [...root.querySelectorAll('.villageList .dropContainer .listEntry, .villageList .listEntry, .listEntry[data-did]')] : [];
                const entry = entries.find(el => String(el.getAttribute('data-did') || '') === id);
                if (entry) {
                ...
                const target = new URL('dorf1.php?newdid=' + encodeURIComponent(id), location.href).href;
                if (location.href !== target) location.href = target;
                return 'navigate-fallback:' + target;
            })();
        """.trimIndent()
        webView.evaluateJavascript(js, null)
        // Jika onPageFinished tidak terpanggil (misalnya navigasi ditangani history API),
        // cek URL secara berkala dan jangan membaca village lama.
        handler.postDelayed({ collectCurrentVillageData() }, 1200)
    }

    private fun collectCurrentVillageData() {
        if (!villageScanActive) return
        val expectedId = villageScanTargets.getOrNull(villageScanIndex)?.first.orEmpty()
        ...
        val js = """
            (() => {
                ...
                const idMatch = location.href.match(/[?&]newdid=(\\d+)/i);
                const id = idMatch ? idMatch[1] : '';
                if (id !== expectedId) {
                    return JSON.stringify({notReady:true,id,expectedId});
                }
                let name = '';
                const activeEntry = document.querySelector('#sidebarBoxVillagelist .villageList .listEntry.active[data-did], #sidebarBoxVillagelist .villageList .listEntry[data-did].active');
                if (activeEntry && String(activeEntry.getAttribute('data-did') || '') === id) {
                    name = clean(activeEntry.querySelector('.name')?.textContent || '');
                }
                if (!name) {
                    const matchingEntry = [...document.querySelectorAll('#sidebarBoxVillagelist .listEntry[data-did]')]
                        .find(el => String(el.getAttribute('data-did') || '') === id);
                    name = clean(matchingEntry?.querySelector('.name')?.textContent || '');
                }
                ...
                const levelNodes = [
                    ...document.querySelectorAll('#village_map .level, #resourceFieldContainer .level, #resourceFieldContainer [class*="level"], #resourceFieldContainer area[href*="build.php?id="], #resourceFieldContainer a[href*="build.php?id="]')
                ];
                for (const el of levelNodes) {
                    const parts = [
                    ...
                    const matches = [...parts.matchAll(/(?:^|[\s_-])level[_-]?(\d{1,2})(?=$|[\s_-])/ig)];
                    for (const m of matches) {
                        const n = parseInt(m[1], 10);
                        if (Number.isFinite(n) && n >= 0 && n <= 20) seenLevels.add(n);
                    }
                    const plain = clean(el.textContent || '');
                    if (/^\d{1,2}$/.test(plain)) seenLevels.add(parseInt(plain, 10));
                }
                // Fallback: Travian sometimes renders only gidN/levelN overlay classes.
                for (const el of document.querySelectorAll('#village_map [class*="gid"], #village_map [class*="level"]')) {
                    const cls = String(el.className || '');
                    const m = cls.match(/(?:^|\s)level(\d+)(?:\s|$)/i);
                    if (m) seenLevels.add(parseInt(m[1], 10));
                }
                fields.push(...seenLevels);
                const nums = fields.filter(n => Number.isFinite(n));
                const minLevel = nums.length ? Math.min(...nums) : -1;

                const resources = {};
                for (const idr of ['l1','l2','l3','l4']) {
                    const el = document.getElementById(idr);
                    if (el) resources[idr] = clean(el.textContent || '');
                }
                AndroidFarm.onVillageScanResult(JSON.stringify({id,name,minLevel,fields,resources}));
            })();
        """.trimIndent()
        webView.evaluateJavascript(js, null)
    }
    ...
    private fun handleVillageScanResult(rawJson: String) {
        ...
        if (json?.optBoolean("notReady", false) == true) {
            val currentId = json.optString("id").ifBlank { "-" }
            val expectedId = json.optString("expectedId").ifBlank { "-" }
            villageScanPageRetry++
            if (villageScanPageRetry <= 12) {
                farmStatus.text = "Menunggu village ${villageScanIndex + 1}/${villageScanTargets.size}…"
                if (villageScanPageRetry == 1 || villageScanPageRetry % 4 == 0) {
                    logEvent("UI: belum pindah ke village target (current=$currentId, target=$expectedId), retry $villageScanPageRetry/12")
                }
                handler.postDelayed({ collectCurrentVillageData() }, 500)
            } else {
                logEvent("UI: gagal pindah ke village target ID $expectedId setelah 12 retry; paksa navigasi ulang")
                villageScanPageRetry = 0
                val forceUrl = "dorf1.php?newdid=$expectedId"
                webView.loadUrl(forceUrl)
                handler.postDelayed({ collectCurrentVillageData() }, 1400)
            }
            return
        }
        villageScanPageRetry = 0
        val expectedId = villageScanTargets.getOrNull(villageScanIndex)?.first.orEmpty()
        val id = json?.optString("id").orEmpty().ifBlank { expectedId }
        val targetName = villageScanTargets.getOrNull(villageScanIndex)?.second.orEmpty()
        val name = json?.optString("name").orEmpty().trim().ifBlank { targetName.ifBlank { "Village $id" } }
        ...
        val actualId = id.ifBlank { expectedId }
        val existingIndex = villageScanResults.indexOfFirst { it.first == actualId }
        if (actualId.isNotBlank()) {
            if (existingIndex >= 0) villageScanResults[existingIndex] = actualId to name
            else villageScanResults.add(actualId to name)
        }

        val progress = "${villageScanIndex + 1}/${villageScanTargets.size}"
        logEvent("UI: village $progress selesai — $name (ID $actualId); min field=$minLevel${if (resourceText.isNotBlank()) "; res: $resourceText" else ""}")
        farmStatus.text = "Village $progress: $name${if (minLevel >= 0) " — field terendah L$minLevel" else ""}"

        villageScanIndex++
        handler.postDelayed({ visitNextVillageForScan() }, 450)
    }

    private fun finishVillageScan() {
        villageScanActive = false
        val unique = villageScanResults.distinctBy { it.first }
        if (villageScanExpected > 0 && unique.size < villageScanExpected) {
            farmStatus.text = "Scan selesai ${unique.size}/${villageScanExpected}; daftar belum lengkap."
            logEvent("UI: scan village selesai tetapi belum lengkap ${unique.size}/${villageScanExpected}")
        } else {
            farmStatus.text = "${unique.size} village selesai dicek dan siap dipilih."
            logEvent("UI: scan village selesai; ${unique.size} village berhasil dicek satu per satu")
        }
