...
class MainActivity : Activity() {
    ...
    private fun collectVillageTargetsFromSidebar() {
        ...
        val js = """
            (() => {
                ...
                if (!root) {
                    AndroidFarm.onVillageTargetsResult(JSON.stringify({villages:[], expected:0}));
                    return;
                }
    ...
    private fun handleVillageTargetsResult(rawJson: String) {
        ...
        if (unique.isEmpty()) {
            villageScanActive = false
            farmStatus.text = "Village tidak ditemukan."
            logEvent("UI: dorf1.php tidak memberikan daftar village")
            return
        }
